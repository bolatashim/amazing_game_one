...
function start() {
	...
	startLabel = new createjs.Text("Go", "bold 80px Arial", "#ff3f22");
	startLabel.x = stage.canvas.width / 2 - 60;
	startLabel.y = stage.canvas.height / 2 - 50;
	
	startButton.fillCmd = startButton.graphics.beginFill("#ff8f7d").command;

	startButton.graphics.drawRect(-200, -100, 400, 200);
...
function gameOver() {
	...
	restartLabel = new createjs.Text("Go again", "bold 75px Arial", "#ff3f22");
	restartLabel.x = stage.canvas.width / 2 - 160;
	restartLabel.y = stage.canvas.height / 2 - 50;
	
	endLabel = new createjs.Text("Game Over :(", "bold 60px Arial", "#ff3f22");
	endLabel.x = stage.canvas.width / 2 - 200;
	endLabel.y = stage.canvas.height / 2 - 250;

	endLabel2 = new createjs.Text("Score: " + heroScore, "bold 50px Arial", "#ff3f22");
	endLabel2.x = stage.canvas.width / 2 - 120;
	endLabel2.y = stage.canvas.height / 2 - 180;
	
	restartButton.fillCmd = restartButton.graphics.beginFill("#ff8f7d").command;


	restartButton.graphics.drawRect(-200, -100, 400, 200);
	console.log(stage.canvas.width / 2);
...
function placeEnemiesOnMap(stage, enemynum) {
   ...
   for (var j = 0; j < enemynum; j++) {
      ...
      var center = Math.random() * (stage.canvas.width - rangeSize) + rangeSize / 2;
      var monster = new enemy([50, 50], yPos, [center - rangeSize/2, center + rangeSize/2], 1, 500);
//////////////////////////////////////////////////////////////////////////////////////////////////
...
function tick(event) {
   ...
   //               allEnemies.splice(i, 1);
            ...
            var yscope = (stage.canvas.height) / 6;
            var yPos = yscope/2 + yscope * i;
            var j = i;
            var rangeSize = Math.random() * 150 + 150;
            var center = Math.random() * (stage.canvas.width - rangeSize) + rangeSize / 2;
            var monster = new enemy([50, 50], yPos, [center - rangeSize/2, center + rangeSize/2], 1, 500);
            monster.appear(stage);
            allEnemies[i] = monster;
            timer22 = 0;
      }else {timer22 ++;}//////////////////////
